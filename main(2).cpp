...
//class dlya zelii
class potions : public coord {
  public:
  int po_hp;
  int po_armor;
  bool is_not_use;
  
  void spawn_potions(room start);
  void draw_potions();
  void action_potions(player* pl);

  potions(int p_hp, int p_armor) {
    po_hp = p_hp;
    po_armor = p_armor;
    is_not_use = true;
  }
...
void potions:: action_potions(player* pl)  {
  if(is_not_use){ // Функция активна только при условии того, что zelie not use
    if((pl->x == x) && (pl->y == y)) {
      
      pl->cur_hp = pl->cur_hp + po_hp;
      po_hp = 0;
      pl->armor = pl->armor + po_armor;
      po_armor = 0;
...
// Просчёт координат zelya
void potions::spawn_potions(room start) {
  x = (rand() % start.size_x) + start.x;
  y = (rand() % start.size_y) + start.y;
...
// Нанесение zelii на экран консоли
void potions:: draw_potions() {
  if(is_not_use) {
    start_color();
...
void draw_all(int rows, int cols, room start, room lvl1, room lvl2, room lvl3, room lvl4, coord quest, player pl, mob test_mob, potions test_potions) {
   //отрисовываем карту
    ...
    test_potions.draw_potions(); // Рисуем зелье
...
int main() {
  ...
  potions test_potions(10, 0); // тестовое зелье
  ...
  test_potions.spawn_potions(start); //  рандомим координаты zelii
  test_potions.spawn_potions(lvl2);
  test_potions.spawn_potions(lvl4);
  ...
  do {
    draw_all(rows, cols, start, lvl1, lvl2, lvl3, lvl4, quest, pl, test_mob, test_potions);
    ...
    test_potions.action_potions(&pl); // deistvie зелья
    ...
    if(action == 32 && (abs(pl.x - test_mob.x) <= 1) && (abs(pl.y - test_mob.y) <= 1)){ // если нажали клавишу атаки(пробел) ?????????
